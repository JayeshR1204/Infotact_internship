...
export const streamPayslipPDF = (
    ...
    doc.fillColor(MUTED).fontSize(8).font('Helvetica')
        .text(
            ...
            'For discrepancies, contact HR at hr@infotact.com | Infotact Solutions Pvt. Ltd.',
            50, rowY, { align: 'center', width: pageWidth }
        );
...
// Keep a JSON payload builder for /api/payroll/download-json if needed
export interface ExportablePayslip {
    documentId: string;
    generatedTimestamp: string;
    corporateHeader: string;
    meta: { employeeName: string; employeeRef: string; payPeriod: string; department: string; };
    financialBreakdown: {
        earnings: { base: number; allowances: number; gross: number; };
